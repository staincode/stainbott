module.exports.run = async (client, message, args) => {
    ...
    return message
      .channel.send({ content: "> ❌ **Başarısız!** Ban yetkin yok dostum." })
      .catch((err) => {});
  if (!message.guild.me.permissions.has("BAN_MEMBERS"))
    return message
      .channel.send({ content: "> ❌ **Başarısız!** Benim ban yetkim yok loooooooooooooo"})
      .catch((err) => {});
  ...
  if (!member)
    return message
      .channel.send({
        content: "> ❌ **Başarısız!** Kullanıcı istiyorum ben, bana açım açım."
      })
      .catch((err) => {});
  ...
  message.guild.members
    ...
    .then(() => {
      message.channel.send({
        content: `> ✅ **(#STAİNYÖNETİM)Başarılı!** Kullanıcı başarıyla sunucudan banlandı!`
      });
    })
    .catch((e) => {
      message
